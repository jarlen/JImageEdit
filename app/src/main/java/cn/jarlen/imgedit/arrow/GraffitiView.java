...
*/
    ...
    // 保存涂鸦操作，便于撤销
    private CopyOnWriteArrayList<GraffitiPath> mPathStack = new CopyOnWriteArrayList<GraffitiPath>();//操作记录列表
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                moveX = (int) Math.abs(event.getX() - downX);//X轴距离
                moveY = (int) Math.abs(event.getY() - downY);//y轴距离
     ...
     * 撤销
     */
    public void undo() {//从当期列表中去除，添加到undo list中
        if (mPathStack.size() > 0) {
            undoStack.add(mPathStack.get(mPathStack.size() - 1));//添加到undo列表
            mPathStack.remove(mPathStack.size() - 1);//移除列表
            initCanvas();
    ...
    public void restore() {//从undo list去除，添加到 mPathStack中
        if (undoStack.size() > 0) {
            mPathStack.add(undoStack.get(undoStack.size() - 1));//添加到列表中
            undoStack.remove(undoStack.size() - 1);//撤销列表中移除
            initCanvas();
            draw(mBitmapCanvas, mPathStack);
            invalidate();
        }
